...
def sistema_admin():
	...
	if form.validate_on_submit():
		...
		print('LISTA EXTRAIDA', lista_extaida)
		...
		#nomeloja=request.form.get('seletor_loja')
...
def sistema(): 
	...
	return render_template('sistema.html', formulario=formulario, query=query, soma_itens=soma_itens[0], 
											soma_valores=soma_valores[0], soma_total=soma_total[0]) 
...
'''
       ...
       <select name="seletor_loja" method="GET" class="custom-select" selected>
                                {% for seletor_loja in lista_loja[1:] %}
                                <option value="{{ seletor_loja }}">{{ seletor_loja }}</option>
                                {% endfor %}
...
'''
    <div class="row">
        <div class="col-md-12">
            <br>
            <h6 class="text-left">Pesquisar itens</h6> </br>
            <form action="SistemaBusca" class="navbar-pull-right">
                <input type="text" class="span2">
                <button class="btn btn-light btn-sm" href="#" method="GET">Buscar</button>
            </form>
        </div>
    </div>
